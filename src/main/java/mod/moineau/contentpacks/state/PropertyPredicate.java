...
public sealed abstract class PropertyPredicate implements Predicate<StateHolder<?, ?>> {
    ...
    public static final class Unbaked extends PropertyPredicate {
        ...
        @Override
        public boolean test(StateHolder<?, ?> state) {
        ...
        private <T extends Comparable<T>> DataResult<Property.Value<T>> bakeValue(Property<T> property) {
            return property.getValue(this.value).map(property::value).map(DataResult::success)
    ...
    public static final class Baked<T extends Comparable<T>> extends PropertyPredicate {
        ...
        @Override
        public boolean test(StateHolder<?, ?> state) {
            return state.getOptionalValue(this.property).map(value -> this.comparator.compare(value, this.value)).orElse(false);
        }

        @Override
        public String toString() {
            return property.getName() + this.comparator + property.getName(this.value);
        }
    }
}
