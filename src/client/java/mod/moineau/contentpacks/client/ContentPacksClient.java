...
public final class ContentPacksClient implements ClientModInitializer {
	...
	private List<ContentPacksClientExtension> extensions;
	...
	public void onInitializeClient() {
		...
		ContentPacks.getInstance().registerLoader(this.colorResolverManager);
		//
		LOGGER.debug("Initializing client extensions...");
		this.extensions = FabricLoader.getInstance().getEntrypoints("contentpacks.client", ContentPacksClientExtension.class);
		this.extensions.forEach(ContentPacksClientExtension::onInitialize);
		//
		ContentPacks.getInstance().loadRepository(packRepository);
		this.extensions.forEach(ContentPacksClientExtension::onContentLoaded);
	}
